...
class Validator
{
     ...
     *
     * @param  integer $id Si un id est fourni l'entrée existe déjà en BDD.
    ...
    public function validateValues($errors = [], $uniqueProperties = [])
    {
        foreach ($this->data as $name => $value) {
            ...
            if (!empty($propErrors)) {
                $errors[$name] = new PropertyValidationErrors($name, $value, get_class($this->model), $propErrors);
            }
    ...
    public function validateProperty($propValue, $propAttributes)
    {
        ...
        if (in_array($propValue, [null, ""], true) && empty($propAttributes['required'])) {
            return [];
