...
class DAO extends PDO
{
     ...
     * @return PDOStatement
     */
    final public function statement($query)
    {
     ...
     */
    final public function execute($sql, $values = [])
    {
        try {
     ...
     * @throws DomainException
     */
    final public function executeSelect($query, $values = [], $className = null)
    {
        try {
     ...
     * @throws DomainException
     */
    final public function executeInsert($query, $values)
    {
        try {
            $statement = $this->statement($query);
     ...
     */
    final public function executeUpdate($query, $values)
    {
     ...
     */
    final public function executeDelete($query, $values)
    {
        return $this->execute($query, $values);
    }
     ...
     * @throws DomainException
     */
    final public function executeInsertProcedure($procedureQuery)
    {
        try {
            return $this->exec($procedureQuery) === 0;
        } catch (PDOException $e) {
            $this->handleException($e, "INSERT procédure");
        }
    }
     ...
     * @throws DomainException
     */
    final public function executeDeleteProcedure($name)
    {
        try {
            return $this->exec("DROP PROCEDURE IF EXISTS `$name`");
        } catch (PDOException $e) {
            $this->handleException($e, "DELETE procédure '$name'");
        }
    }
     ...
     * @throws DomainException
     */
    final public function executeCallProcedure($name, $params = [])
    {
        try {
            $params = join(', ', $params);
            return $this->execute("CALL $name($params)") !== false;
        } catch (PDOException $e) {
            $this->handleException($e, "CALL procédure '$name'");
        }
    }
     ...
     */
    final public function executeChecksum($query)
    {
     ...
     * @throws DomainException
     */
    private function handleException(PDOException $e, $reqName = "")
    {
