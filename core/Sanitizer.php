...
class Sanitizer
{
    ...
    public function sanitize(array $optAttributes = [])
    {
        foreach ($this->data as $name => $value) {
            // Suppression des propriétés sans attribut
            if (empty($this->attributes[$name]) && empty($optAttributes[$name]) && $this->strict) {
                unset($this->data[$name]);
                continue;
            }

            // Récupération des attributs de la propriété
            $propAttributes = empty($this->attributes[$name]) ? [] : $this->attributes[$name];

            // Ajout des attributs additionnels
            if (!empty($optAttributes[$name])) {
                $propAttributes = array_merge($propAttributes, $optAttributes[$name]);
            }

            // Cas de valeur vide
            if ($value === "" && !in_array($this->attributes[$name]["type"], ["string", "boolean"])) {
                if (isset($propAttributes['nullable']) && $propAttributes['nullable'] === true) {
                    $this->data[$name] = null;
                } else {
                    unset($this->data[$name]);
                }
